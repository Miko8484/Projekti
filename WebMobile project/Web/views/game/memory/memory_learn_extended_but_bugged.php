...
echo "win pos <br>";
for ($j = 0; $j < $win_possitions_counter; $j++) {
    $string_name = $win_possitions[$j];
    $string_name_decoded = json_decode($string_name);
    print_r($string_name_decoded);
    echo $$string_name . "<br>";
}


// tu je nekši error ka pokaže friendly fields 0

echo "lose pos <br>";
for ($j = 0; $j < $lose_possitions_counter; $j++) {
    $string_name = $lose_possitions[$j];
    $string_name_decoded = json_decode($string_name);
    print_r($string_name_decoded);
    echo $$string_name . "<br>";
}
...
$no_same_bool = true;
echo "same pos <br>";
for ($i = 0; $i < $win_possitions_counter; $i++) {
    $win_string_name = $win_possitions[$i];
    for ($j = 0; $j < $lose_possitions_counter; $j++) {
        $lose_string_name = $lose_possitions[$j];

        if ($win_string_name == $lose_string_name) {
            $string_name_decoded = json_decode($win_string_name);
            print_r($string_name_decoded);
            $no_same_bool = false;
        }
    }
...
$arr_name = "\$owner_count_1x1";
echo "pattern ai function: <br>";

for ($j = 0; $j < $nm_of_stats; $j++) {// verjetno neje dobro dvakrat je enako ka te dvojne točke da gor samo nekši čuden način je ka nebi dao dvojne točke

    if ($difference_count_greater[$j] > 0) {
        echo "if(";
        echo $arr_name . "[" . $j . "]>=" . $avg_win_counter[$j] / ($difference_count_greater[$j] + $difference_count_smaller[$j]);
        echo ") \$priority +=";
        echo $difference_count_greater[$j] . ";";
        echo "<br>";
    }
    if ($difference_count_smaller[$j] > 0) {
        echo "if(";
        echo $arr_name . "[" . $j . "]<=" . $avg_win_counter[$j] / ($difference_count_greater[$j] + $difference_count_smaller[$j]);
        echo ") \$priority +=";
        echo $difference_count_smaller[$j] . ";";
...
if ($no_same_bool == true) {
    for ($i = 0; $i < $win_possitions_counter; $i++) {
        $string_name = $win_possitions[$i];
        $string_name_decoded = json_decode($string_name);
        echo "if(";
        for ($j = 0; $j < $nm_of_stats; $j++) {
            echo $arr_name . "[" . $j . "]==" . $string_name_decoded[$j];
            if ($j < ($nm_of_stats - 1)) echo " and ";
        }
        echo ") \$priority +=";
        echo $$string_name . ";";
        echo "<br>";
...
function check_stats($win_statistics, $lose_statistics_arr, $times_lost)
{
    ...
    for ($i = 0; $i < $times_lost; $i++) {
        for ($j = 0; $j < $nm_of_stats; $j++) {
            if ($win_statistics[$j] > $lose_statistics_arr[$i][$j]) {
                $difference_count_greater[$j]++;
                $avg_win_counter[$j] += $win_statistics[$j];
            } else if ($win_statistics[$j] < $lose_statistics_arr[$i][$j]) {
